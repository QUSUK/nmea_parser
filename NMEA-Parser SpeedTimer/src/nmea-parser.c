	...
	for (uint8_t Counter = 0; Token != NULL; Counter++)
	{
		*Token = '\0';
		...
		case 2:
			...
			break;
		case 3:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%lf", &NMEA_Struct->RMCData->Latitude);
			break;
		case 4:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%c", &NMEA_Struct->RMCData->NS);
			break;
		case 5:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%lf", &NMEA_Struct->RMCData->Longitude);
			break;
		case 6:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%c", &NMEA_Struct->RMCData->EW);
			break;
		case 7:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%lf", &NMEA_Struct->RMCData->Speed);
			break;
		case 8:
			break;
		case 9:
	...
	for (uint8_t Counter = 0; Token != NULL; Counter++)
	{
		*Token = '\0';
		...
		case 1:
			...
			break;
		case 2:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%lf", &NMEA_Struct->GGAData->Latitude);
			break;
		case 3:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%c", &NMEA_Struct->GGAData->NS);
			break;
		case 4:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%lf", &NMEA_Struct->GGAData->Longitude);
			break;
		case 5:
		...
		case 7:
			break;
		case 8:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%lf", &NMEA_Struct->GGAData->HDOP);
			break;
		case 9:
			Num_Scan_Chars += sscanf((const char*)NMEA_Data, "%lf", &NMEA_Struct->GGAData->Height);
			break;
		case 10:
...
void Convert_Coords_To_Degs(int8_t* Degs, double* Latitude, double* Longitude, uint8_t NS, uint8_t EW)
{
	...
	Lat_Deg = (int)*Latitude / 100 + fmod(*Latitude, 100) / 60.0;
	Lat_Deg = (NS == 'N') ? +Lat_Deg : -Lat_Deg;

	Lon_Deg = (int)*Longitude / 100 + fmod(*Longitude, 100) / 60.0;
